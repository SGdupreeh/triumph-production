...
(function(){
    ...
    document.addEventListener('touchmove', function(e) {
        if (e.touches.length > 1) {
            e.preventDefault(); // Блокируем щипок (pinch)
        }
    }, { passive: false });
    ...
    document.addEventListener('touchend', function(e) {
        ...
        if (now - lastTouchEnd <= 300) {
            e.preventDefault(); // Блокируем двойной тап (double tap zoom)
        }
        lastTouchEnd = now;
    }, { passive: false });

    /* ── iOS Video Autoplay Force ── */
    ...
    const vobs = new IntersectionObserver(entries => {
        entries.forEach(e => {
            if (e.isIntersecting) {
                ...
                if (!isMobile) e.target.pause(); // Не ставим на паузу на мобильных, чтобы не сломать iOS
            }
        });
    }, { threshold: isMobile ? 0.05 : 0.15 });
    
    document.querySelectorAll('.vcard__vid video, .sqcard__vid video').forEach(v => { 
        if (!isMobile) v.pause(); // Не тормозим загрузку на мобильных
        vobs.observe(v); 
    });
